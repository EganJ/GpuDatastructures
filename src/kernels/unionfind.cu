 ...
 */
__device__ int gpuds::unionfind::get_class(int *classes, int i)
{
 ...
 */
__global__ void gpuds::unionfind::kernel_mass_merge(int *classes, const int *a, const int *b, int n)
{
...
void gpuds::unionfind::mass_merge(int *classes, const int *a, const int *b, int n)
{
  int blockSize = 32;
...
__global__ void gpuds::unionfind::kernel_flatten(int *classes, int n)
{
  int idx = blockIdx.x * blockDim.x + threadIdx.x;
  int work = impl::ceildiv(n, gridDim.x * blockDim.x);
...
void gpuds::unionfind::flatten(int *classes, int n)
{
  int blockSize = 256;
  int numBlocks = (n + blockSize - 1) / blockSize;
